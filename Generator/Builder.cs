...
public class Builder
{
        ...
        params char[] additionalSymbols)
    {
        ...
        {
            var swappedSpan = additionalSymbols.Select(x => (int) x).ToArray().AsSpan();
            UpdateResult(ref generalSpan, ref swappedSpan);
            swappedSpan.Clear();
        }
    ...
    {
        var generalSpanCopy = new Span<int>(new int[span.Length + newSpan.Length]);
